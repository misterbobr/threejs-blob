...
function init() {
    ...
    for (let i = 0; i < meshes.length; i++) {
        meshes[i] = new THREE.Mesh(geometry2, material);
    }
    ...
    intersectedObjects = Array.from(meshes, () => false);
}
...
function tick() {
    ...
    meshes[0].geometry.attributes.position.needsUpdate = true;
    meshes[0].geometry.normalsNeedUpdate = true;
    ...
    if (!intersects.includes(meshes[0])) {
        if (intersectedObjects[0]) {
            // meshes[0].geometry.scale(1/1.2, 1/1.2, 1/1.2);
            intersectedObjects[0] = false;
        }
        meshes[0].material.color.set(defaultColor);
        // aLight.intensity = 0;
    }
    ...
    for (let i = 0; i < intersects.length; i++) {
        ...
        if (!intersectedObjects[i]) {
            intersects[i].object.geometry.scale(1.2, 1.2, 1.2);
            intersectedObjects[i] = true;
